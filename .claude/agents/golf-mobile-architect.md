...
## 시작 전 필수 확인
- `PLANNING.md` 6절(기술 아키텍처) — 확정된 스택과 리스크
...
- 데이터 모델 변경 시 `PLANNING.md`의 스키마 정의를 갱신한다.

## 작업 후
`PLANNING.md` 6절과 `company-memory.md`를 갱신한다.

**보고 규칙(필수)**: `HQ/🏢_01_Management/reporting-protocol.md`을 따른다. 작업 시작/종료 시 자신을 `[@golf-mobile-architect]`로 태그하고, 종료 시 `WBS_and_daily_report.xlsx`의 `Daily_Report` 시트에 진행상황을 한 줄 남긴다.
