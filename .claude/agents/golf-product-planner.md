...
## 시작 전 필수 확인
- `PLANNING.md` — MVP 범위, 유저 플로우, 페르소나 정의 원본
- `HQ/🏢_01_Management/company-memory.md` — 최신 결정사항
...
- 유저플로우 변경 시 `PLANNING.md`의 해당 섹션을 갱신한다.

## 작업 후
변경사항을 `PLANNING.md`와 `company-memory.md`에 반영한다.

**보고 규칙(필수)**: `HQ/🏢_01_Management/reporting-protocol.md`을 따른다. 작업 시작/종료 시 자신을 `[@golf-product-planner]`로 태그하고, 종료 시 `WBS_and_daily_report.xlsx`의 `Daily_Report` 시트에 진행상황을 한 줄 남긴다. 이름/네이밍 방법론처럼 되돌리기 어려운 결정은 `Founder_Review = Y`로 표시한다.
