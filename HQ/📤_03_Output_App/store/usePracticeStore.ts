...
type PracticeState = {
   ...
   *
   * ⚠️ 재생 배속(playbackRate)은 **여기서 파생**시킨다. 별도 `rate` 필드를
   * 두지 않는 이유: 예전엔 rate와 속도를 둘 다 상태로 들고 있었는데, 속도만
   * 영속되고 rate는 안 되니 앱을 재시작하면 "빠르게로 표시되는데 소리는 보통"인
   * 어긋남이 생겼다. 진실의 출처를 하나로 줄여 그 버그 자체를 없앤다.
   * → 배속이 필요하면 `getSwingSpeed(swingSpeed).rate`.
   */
