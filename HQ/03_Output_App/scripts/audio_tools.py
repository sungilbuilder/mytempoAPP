...
def room_tone(n: int, level_db: float = -80.0, rng: np.random.Generator | None = None,
    ...
    측정해보니 루프의 **26.9%가 -60dBFS 이하 절대 무음**이었다.

    ⚠️ 다만 이 함수가 '공간감'을 만드는 건 아니다. 그 일은 `reverb()`가 한다.
    여기가 하는 건 **진폭이 정확히 0인 구간을 없애는 것**뿐이고, 레벨도
    들리지 않을 만큼(-80dBFS) 낮게 쓴다. 밝은 잡음은 같은 크기여도 '히스'로
    인지되므로 1.5kHz 로우패스로 어둡게 만든다 — 조용한 앱에서 히스가
    들리면 잃는 게 훨씬 크다.
    """
...
def hit_similarity(x: np.ndarray, positions, win: float = 0.05,
                   highpass: float = 1000.0, sr: int = SR) -> float:
    """
    타격들의 **어택 잡음층**이 서로 얼마나 같은가 (0~1).
    라운드로빈이 실제로 걸렸는지 본다.

    ## 왜 재는가

    측정 당시 1번 타격과 2번 타격의 정규화 상관계수가 **0.9948**이었다.
    `np.random.seed(7)` 하나로 전역 고정하고 같은 함수를 부르니, 세 마커가
    **게인만 다른 동일 파형**이었던 것이다. 파일 자체도 루프라 2초마다
    ...
    ## 창을 왜 두 번 좁혔나 — 측정이 두 번 틀렸다

    ① 처음엔 180ms 전체를 쟀다. 변주를 넣었는데도 값이 0.996으로 그대로였다.
       창이 길면 값의 대부분이 **바디(지속되는 165Hz 사인)** 에서 나오는데,
       바디는 원래 같아야 한다. 팩의 음정이 타격마다 흔들리면 그게 더 싸구려다.

    ② 창을 어택 50ms로 좁혔더니 나무 0.94 / 북 0.98. 여전히 높았다.
       50ms 안에도 기음이 8주기 들어 있어 그 결정론적 사인이 값을 지배했다.

    변주가 필요한 건 그 위에 얹히는 **타격 잡음층**이다. 그래서 1kHz 위만
    떼어 잰다. 현 팩(Karplus-Strong의 초기 잡음이 매번 다르다)이 0.10으로
    나오는 게 정상 동작의 기준점이다.

    각 타격 구간을 정규화해 짝별 상관계수를 구하고 그 **최댓값**을 돌려준다.
    """
    x = np.asarray(x, dtype=np.float64)
    if highpass > 0:
        spec = np.fft.rfft(x)
